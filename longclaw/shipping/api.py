...
from longclaw.shipping import models, serializers, utils
...
def shipping_cost(request):
    ...
    try:
        ...
        try:
            ...
            data = {"message": "Shipping option {} is invalid".format(kwargs["name"])}
        except utils.InvalidShippingCountry:
            data = {
                "message": "Shipping to {} is not available".format(
                    kwargs["country_code"]
                )
            }
        else:
...
def shipping_options(request, country=None):
    ...
    if processors:
        if not destination:
            return Response(
                data={
                    "message": "Destination address is required for rates to {}.".format(
                        country_code
                    )
                },
        ...
        for processor in processors:
            processor.get_rates(
                settings=settings, basket_id=bid, destination=destination
            )
